...
def get_or_create_secret_key() -> str:
    ...
    Called once, in the gunicorn master process before it forks workers
    (see the --preload flag in the Dockerfile) — so every worker ends up
    with the exact same key instead of each one racing to generate its own
    on a fresh cache, which would make cookies work only against whichever
    worker happened to issue them."""
